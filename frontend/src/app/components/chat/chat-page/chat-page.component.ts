...
@Component({
  ...
  imports: [
    ...
    ChatSidebarComponent, ChatHeaderComponent,
    MessageListComponent, ChatComposerComponent, EmptyStateComponent
  ],
...
export class ChatPageComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit(): Promise<void> {
    ...
    try { await this.chatService.startSignalR(); } catch (e) { console.error(e); }
  ...
  openChat(chat: ChatListItem): void {
    ...
    this.chats = this.chats.map(c => c.id === chat.id ? { ...c, unreadCount: 0 } : c);
  ...
  closeChat(): void {
    ...
    this.router.navigate([], { queryParams: { chatId: null }, queryParamsHandling: 'merge' });
  }

  async sendMessage(text: string): Promise<void> {
    if (!text.trim() || !this.selectedChat) return;
    try { await this.chatService.sendMessage(this.selectedChat.id, text); }
  ...
  async onDeleteMessage(messageId: string): Promise<void> {
    ...
    try { await this.chatService.deleteMessage(messageId); }
    catch { alert('Не вдалося видалити повідомлення'); }
  }
  ...
  private showTypingIndicator(): void {
    ...
    if (this.typingTimer) clearTimeout(this.typingTimer);
    this.typingTimer = setTimeout(() => { this.typingText = null; }, 3000);
  }
  ...
  private updateLocalReadStatus(lastReadId: string): void {
    ...
    this.messages = this.messages.map((m, i) =>
      i <= idx && m.senderId === this.userId ? { ...m, isRead: true } : m
    );
  }
  // Inside your component class:

  onSidebarChatDeleted(deletedChatId: string | number): void {
    // Ensure the type matches whatever $event emits (likely a string or number ID)
